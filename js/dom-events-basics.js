...
//
    function setAnotherOneEvent (){
        let anotherNode = document.getElementsByClassName('dropdown-item');
        console.log(anotherNode);
        anotherNode[2].addEventListener('click', appendAnotherOneDiv);
    }
    setAnotherOneEvent();

    function appendAnotherOneDiv () {
        let appendingDiv = document.getElementById('another-one')
                appendingDiv.appendChild(anotherOneGif());
                //Will remove the clock event after it is clicked once and appended
                removeImageClickEvent();
    }

    function anotherOneGif () {
        let djKhaled = document.createElement('img')
            djKhaled.setAttribute('src', "assets/dj-khaled.gif")
            djKhaled.setAttribute('class', 'col-12');
            return djKhaled;
    }


    //HOW TO REMOVE THE EVENT
// 1. Find out where the event is coming from
function removeImageClickEvent () {
        let sus = document.getElementById('another-one-anchor')
